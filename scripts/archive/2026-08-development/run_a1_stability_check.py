#!/usr/bin/env python3
"""A1 development stability check: 10 anchors only (not dissertation findings).

Frozen formal A1 model config + prompt a1_oneshot_v2_diversified.
Does not modify A0, A2, D1, governance, budgets, or existing outputs.
Does not run the 100-anchor evaluation.
"""
...
N_STABILITY_ANCHORS = 10


def select_stability_anchors(
    ...
    *,
    ...
    digest = hashlib.sha256(
        f"{int(experiment_seed)}:a1_stability_anchor_selection".encode("utf-8")
    ).hexdigest()
    ...
    *,
    ...
    if not raw_path.exists():
        raise FileNotFoundError(
            ...
            "Mount the external drive before running the A1 stability check."
        )
    ...
    pool_cfg = ReferencePoolConfig(
        ...
        label="a1_stability_reference_pool",
        source_path=pool_cfg.source_path,
    )
    provider = ReferencePoolProvider.from_config(pool_cfg, raw_path=raw_path)
    budget_spec = budget.to_budget_spec(label="a1_stability_budget_interface")
    ...
    for anchor_id in anchor_ids:
        ...
        logger = TrajectoryLogger(
            ...
            run_id=f"a1_stability_{anchor_id}",
        )
        ...
        if plan is not None:
            for item in plan.get("frozen_candidates", []):
                ...
                if label is None and isinstance(item.get("strategy_label"), str):
        ...
        per_anchor.append(
            {
                ...
                "selected_response_index": (
                    call.selected_response_index if call is not None else None
                ),
                ...
                "model_config_path": (
                    call.model_config_path if call is not None else None
                ),
            }
        )
    ...
    return {
        "label": "a1_stability_n10",
        "status": "a1_stability_development_check_not_dissertation_findings",
        "n_anchors": n,
        ...
        "asr_curve_descriptive_only": asr,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--n-anchors",
        type=int,
        default=N_STABILITY_ANCHORS,
        choices=(N_STABILITY_ANCHORS,),
        help="Development stability size only: exactly 10 anchors.",
    )
    ...
    if int(args.m) != 2 or int(args.q) != 5:
        raise SystemExit("A1 stability check is locked to Q=5 and m=2 for this stage.")
    if int(args.n_anchors) != N_STABILITY_ANCHORS:
        raise SystemExit("A1 stability check is locked to n=10 anchors.")
    ...
    selected = select_stability_anchors(
        all_ids, n=int(args.n_anchors), experiment_seed=int(args.experiment_seed)
    )

    stamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    run_name = (
        f"a1_stability_v2_n{args.n_anchors}_m{budget.m_max}_q{budget.q_max}_"
        f"seed{args.experiment_seed}_{stamp}"
    )
    run_dir = new_run_directory(
        run_name,
        parent=EXPERIMENTS_ROOT / "a1" / "stability",
        stage="experiments",
    )

    formal = FORMAL_A1_MODEL_CONFIG
    (run_dir / "stability_anchors.json").write_text(
        json.dumps(
            {
                "selection_rule": (
                    "stable_sha256(experiment_seed:a1_stability_anchor_selection) "
    ...
    summary["stability_anchors_path"] = str(run_dir / "stability_anchors.json")
    ...
    lines = [
        "A1 stability n=10 (DEVELOPMENT CHECK — NOT dissertation findings)",
        f"run_dir: {run_dir}",
        f"budget: Q={budget.q_max}, m={budget.m_max}",
        f"formal_model_config: {formal.to_dict()}",
        f"config_hash: {summary['config_hash']}",
        f"prompt_hashes_observed: {summary['prompt_hashes_observed']}",
        f"anchors: {selected}",
        ...
        f"counts={summary['parse_status_counts']}",
        f"governance_valid_rate_overall: "
        f"{summary['governance_valid_rate_overall']}",
        f"budget_exceeded_rate_overall: "
        f"{summary['budget_exceeded_rate_overall']}",
        f"field_pair_diversity: "
        f"{summary['field_pair_diversity']['mean_distinct_field_pairs_per_anchor']:.2f} "
        f"mean distinct/anchor; "
        f"global={summary['field_pair_diversity']['n_global_distinct_field_pairs']}",
        f"strategy_label_diversity: "
        f"{summary['strategy_label_diversity']['mean_distinct_strategy_labels_per_anchor']:.2f} "
        f"mean distinct/anchor; "
        f"global={summary['strategy_label_diversity']['n_global_distinct_strategy_labels']}",
        f"ASR@1..5 (descriptive only): {summary['asr_curve_descriptive_only']}",
    ...
    report = "\n".join(lines) + "\n"
    (run_dir / "stability_report.txt").write_text(report, encoding="utf-8")
    print(report)
    return 0
