#!/usr/bin/env python3
"""Authoritative formal A0--A3 comparison runner.

Currently archived under ``scripts/archive/2026-08-development/`` as the future
formal four-attacker comparison base.  Prefer updating this file (and the
paired-reporting patterns in ``run_a0_a2_paired_dev_comparison.py``) after
provenance fixes; do not invent a parallel runner/orchestrator/logger.

The frozen JSON configuration is the only source of experimental parameters.
Without ``--launch`` this command performs a read-only preflight.  Every launch
creates a new output directory and refuses configuration, code, data, D1,
governance or anchor-set drift.
"""
...
def _build_attacker(
    ...
    *,
    ...
    if attacker_id == "a0":
        return ConstrainedRandomAttacker(
    ...
    if attacker_id == "a2":
        return SurrogateGuidedSearcher(
            budget=budget,
            reference_pool=pool,
            experiment_seed=seed,
            attacker_id="a2",
