#!/usr/bin/env python3
"""Formal A0 E-budget calibration runner (evaluation only).

Freezes the A0 edit-budget curve under shared anchors, reference pools and
seeds.  Writes only under ``05_outputs/experiments/a0/calibration/`` with an
explicit ``stage="experiments"`` gate.

Does not modify A0, governance, budget accounting, environment, or D1.
"""
...
# --- formal calibration configuration ---
ATTACKER = "a0"
DEFENCE = "d1"
N_ANCHORS = 100
REFERENCE_POOL_K = 10
Q_MAX = 5
SEED = 20260803
E_LIST: tuple[int, ...] = (1, 3, 5, 7, 9)

# Same A0 seed rule for every anchor and every E: fixed episode seed = SEED.
A0_SEED_RULE = "fixed_episode_seed_equals_experiment_seed"
...
def git_commit_hash() -> str:
    """Best-effort commit hash from the implementation repository."""
...
def build_pool_config(*, k: int, seed: int) -> ReferencePoolConfig:
    ...
    return ReferencePoolConfig(
        ...
        label="a0_formal_e_budget_calibration_reference_pool",
...
def run_episode(
    *,
    ...
    budget = BudgetSpec.development_dummy(
        q_max=q_max,
        e_max=e_max,
        label="a0_formal_e_budget_calibration",
    )
...
def summarise_e(
    *,
    ...
    failures = Counter(row.stop_reason for row in rows if not row.success)
...
def format_report(
    *,
    ...
    lines = [
        "A0 Formal E Budget Calibration",
        "",
...
def write_curve_csv(
    path: Path, metrics_by_e: Sequence[EBudgetMetrics], q_max: int
) -> None:
    fieldnames = [
        "E_max",
        "n_anchors",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    run_dir = new_run_directory(
        f"a0_e_budget_n{N_ANCHORS}_{stamp}",
    ...
    report = format_report(
        metrics_by_e=metrics_by_e, metadata=metadata, q_max=Q_MAX
    )
    print(report)
