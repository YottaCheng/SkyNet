"""Load DeepSeek API settings from the dissertation-root ``.env`` file.

This module is intentionally separate from A0/A2 attack-lab code. It only
exposes local configuration for API connectivity checks and future LLM wiring.
"""
...
# 04_implementation/src/deepseek_config.py -> dissertation root
DISSERTATION_ROOT = Path(__file__).resolve().parents[2]
DEFAULT_ENV_PATH = DISSERTATION_ROOT / ".env"
...
def load_deepseek_settings(env_path: Path | None = None) -> DeepSeekSettings:
    """Load DeepSeek settings from ``.env``.
    ...
    if not api_key:
        raise RuntimeError(
            "DEEPSEEK_API_KEY is missing. Copy .env.example to .env at the "
            f"dissertation root and set DEEPSEEK_API_KEY (expected file: {path})."
        )
