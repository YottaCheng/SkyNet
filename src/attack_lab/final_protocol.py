...
DEFAULT_PROTOCOL_PATH = (
    Path(__file__).resolve().parents[2] / "config" / "final_month7_protocol.json"
)
...
class FinalProtocolConfig:
    ...
    def validate(self) -> None:
        ...
        if str(secondary.get("role")) != "SECONDARY_PRESPECIFIED":
            errors.append("D2-S v1.1 role must be SECONDARY_PRESPECIFIED")
...
def verify_frozen_artefacts(protocol: FinalProtocolConfig) -> list[str]:
    ...
    d2_primary = Path(p["d2"]["primary"]["artefact_path"])
    if d2_primary.is_file():
...
def protocol_role_statement() -> dict[str, Any]:
    return {
        ...
        "secondary_prespecified": {
            ...
            "description": "D2-S v1.1 Isolation Forest refinement",
        },
